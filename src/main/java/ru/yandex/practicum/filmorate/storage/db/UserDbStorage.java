...
public class UserDbStorage implements UserStorage {
	...
	public List<User> listUsers() {
		...
		return jdbcTemplate.query(sqlUser,
				(rs, rowNum) -> new User(rs.getLong("user_id"), rs.getString("email"), rs.getString("user_name"),
	...
	public User getUserById(Long userId) {
		...
		if (sqlUser.next()) {
			user = new User(sqlUser.getLong("user_id"), sqlUser.getString("email"), sqlUser.getString("user_name"),
					sqlUser.getString("login"), Objects.requireNonNull(sqlUser.getDate("birthday")).toLocalDate(),
	...
	public void updateUserProperties(User user) {
		...
		String sqlQueryUser = "UPDATE users SET " + "email = ?, user_name = ?, login = ?, birthday = ? "
				+ "WHERE user_id = ?";
