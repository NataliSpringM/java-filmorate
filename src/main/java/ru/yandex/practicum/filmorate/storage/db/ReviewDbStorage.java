...
public class ReviewDbStorage implements ReviewStorage {
	...
	public List<Review> listReviews(Integer filmId, Integer count) {
		...
		return jdbcTemplate.query(sqlReview, (rs, rowNum) -> new Review(rs.getInt("review_id"), rs.getString("content"),
				rs.getBoolean("is_positive"), rs.getLong("user_id"), rs.getInt("film_id"), rs.getInt("useful")));
	}
	...
	public Review getReviewById(Integer reviewId) {
		...
		StringBuilder queryBuilder = new StringBuilder();
		queryBuilder.append("SELECT r.*, rl.useful").append(" FROM")
				.append(" (SELECT * FROM reviews WHERE review_id = ").append(reviewId).append(") r").append(" JOIN")
				.append(" (SELECT review_id, SUM(like_or_dislike) AS useful").append(" FROM reviews_likes")
				.append(" GROUP BY review_id) rl").append(" ON r.review_id = rl.review_id");
		String sqlReview = queryBuilder.toString();
		...
		Review review;
		if (sql.next()) {
			review = new Review(sql.getInt("review_id"), sql.getString("content"), sql.getBoolean("is_positive"),
					sql.getLong("user_id"), sql.getInt("film_id"), sql.getInt("useful"));
		} else {
			log.info("Отзыв с идентификатором {} не найден.", reviewId);
			throw new ObjectNotFoundException(String.format("Отзыв с id: %d не найден", reviewId));
		}
