...
public class FriendshipDbStorage implements FriendshipStorage {
	...
	public Set<Long> listUserFriends(Long userId) {
		...
		SqlRowSet sqlQueryListUserFriends = jdbcTemplate
				.queryForRowSet("SELECT recipient_id FROM friendship WHERE initiator_id = ?", userId);
	...
	public Set<Long> listCommonFriends(Long userId, Long otherId) {
		...
		SqlRowSet sqlQueryListCommonFriends = jdbcTemplate.queryForRowSet(
				"SELECT f1.recipient_id FROM friendship AS f1 JOIN friendship AS f2 "
						+ "ON f1.recipient_id = f2.recipient_id " + "WHERE f1.initiator_id = ? AND f2.initiator_id = ?",
				userId, otherId);
