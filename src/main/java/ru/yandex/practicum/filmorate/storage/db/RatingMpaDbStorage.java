...
public class RatingMpaDbStorage implements RatingMpaStorage {
	...
	public List<Mpa> listRatingMpa() {
		...
		return jdbcTemplate.query(sql,
				(rs, rowNum) -> new Mpa(rs.getInt("rating_mpa_id"), rs.getString("rating_mpa_name")));
	...
	public Mpa getRatingMpaById(Integer ratingMpaId) {
		...
		SqlRowSet sqlRatingQuery = jdbcTemplate.queryForRowSet("SELECT * FROM rating_mpa WHERE rating_mpa_id = ?",
				ratingMpaId);

		if (sqlRatingQuery.next()) {

			Mpa mpa = new Mpa(sqlRatingQuery.getInt("rating_mpa_id"), sqlRatingQuery.getString("rating_mpa_name"));
